        ...
        const viewportHandler = (event: Event) => {
            ...
            requestAnimationFrame(() => {
                ...
                if (layoutViewport.getBoundingClientRect().top < 0) {
                    layoutViewport.style.transform = `translate(0, ${-layoutViewport.getBoundingClientRect()
                        .top}px)`
                }
            })
        }
...
export const setPadding = (vditor: IVditor) => {
    ...
    if (vditor.wysiwyg.element.parentElement.style.display !== "none") {
        const padding =
            (vditor.wysiwyg.element.parentElement.clientWidth -
                vditor.options.preview.maxWidth) /
            2
        vditor.wysiwyg.element.style.padding = `10px ${Math.max(
            minPadding,
            padding
        )}px`
    }
    ...
    if (vditor.preview.element.style.display !== "block") {
        vditor.toolbar.element.style.paddingLeft =
            Math.max(
                5,
                parseInt(
                    vditor[vditor.currentMode].element.style.paddingLeft || "0",
                    10
                ) +
...
export const setTypewriterPosition = (vditor: IVditor) => {
    ...
    vditor[vditor.currentMode].element.style.setProperty(
        "--editor-bottom",
