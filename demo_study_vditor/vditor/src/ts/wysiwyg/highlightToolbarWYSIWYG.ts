...
export const highlightToolbarWYSIWYG = (vditor: IVditor) => {
    ...
    vditor.wysiwyg.hlToolbarTimeoutId = window.setTimeout(() => {
        ...
        const isBlock = blockRenderElement
        ...
        vditor.wysiwyg.element
            ...
            .forEach((itemElement) => {
                if (
                    ...
                    (blockRenderElement &&
                        isBlock &&
                        !blockRenderElement.contains(itemElement))
                ) {
                    const previousElement =
                        itemElement.previousElementSibling as HTMLElement
...
const genUp = (range: Range, element: HTMLElement, vditor: IVditor) => {
    ...
    upElement.setAttribute(
        "aria-label",
        window.VditorI18n.up + "<" + updateHotkeyTip("⇧⌘U") + ">"
    )
...
const genClose = (element: HTMLElement, vditor: IVditor) => {
    ...
    close.setAttribute(
        "aria-label",
        window.VditorI18n.remove + "<" + updateHotkeyTip("⇧⌘X") + ">"
    )
    close.innerHTML =
