...
// removeCurrentToolbar和setCurrentToolbar差不多，都是在
// element存在时，然后添加或删除
export const removeCurrentToolbar = (
    ...
    names.forEach((name) => {
        ...
        if (
            itemElement &&
            itemElement.classList.contains("vditor-menu--current")
        ) {
...
export const setCurrentToolbar = (
    toolbar: { [key: string]: HTMLElement },
    names: string[]
) => {
    names.forEach((name) => {
...
export const enableToolbar = (
    ...
    names.forEach((name) => {
        ...
        if (
            itemElement &&
            itemElement.classList.contains(Constants.CLASS_MENU_DISABLED)
        ) {
...
export const hidePanel = (
    ...
    if (panels.includes("subToolbar")) {
        ...
        if (vditor.toolbar.elements.emoji) {
            ;(
                vditor.toolbar.elements.emoji.lastElementChild as HTMLElement
            ).style.display = "none"
        }
    }
...
export const toggleSubMenu = (
    ...
    actionBtn.addEventListener(getEventName(), (event) => {
        ...
        if (panelElement.style.display === "block") {
            ...
            if (
                vditor.toolbar.element.getBoundingClientRect().right -
                    actionBtn.getBoundingClientRect().right <
                250
            ) {
