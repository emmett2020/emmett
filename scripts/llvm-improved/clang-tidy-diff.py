...
see https://github.com/llvm/llvm-project/blob/main/clang-tools-extra/clang-tidy/tool/clang-tidy-diff.py for latest version.
"""
...
def run_tidy(task_queue, lock, timeout, result):
  watchdog = None
  while True:
...
# To collect returncode returned by subprocess.
result = []

def start_workers(max_tasks, tidy_caller, task_queue, lock, timeout):
  for _ in range(max_tasks):
    t = threading.Thread(target=tidy_caller, args=(task_queue, lock, timeout, result))
    t.daemon = True
    t.start()
...
def main():
  ...
  if args.config_file:
    common_clang_tidy_args.append('--config-file=%s' % args.config_file)
  ...
  # return whether at least one check fails
  if 1 in result:
    sys.exit(1)
