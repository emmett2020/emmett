...
__global__ void block_naive(float* A, float* B, float* C, int M, int N, int K) {
  //一个block内的所有thread每次能读取的元素个数。
  ...
  for (int idx = 0; idx < K / BK; ++idx) {
    for (int i = 0; i < BM * BK / STRIDE; ++i) {
      ...
      s_a[sa_row][sa_col] = A[(blockIdx.y * BM + sa_row) * K + sa_col + idx * BK];
    }
    for (int i = 0; i < BN * BK / STRIDE; ++i) {
      int offset          = i * STRIDE + OFFSET;
      int sb_row          = offset / BN;
      int sb_col          = offset % BN;
      s_b[sb_row][sb_col] = B[(sb_row + idx * BK) * N + blockIdx.x * BN + sb_col];
    }
    __syncthreads();
  ...
  for (int i = 0; i < TM; ++i) {
    for (int j = 0; j < TN; ++j) {
      C[(blockIdx.y * BM + threadIdx.y * TM + i) * N + blockIdx.x * BN + threadIdx.x * TN + j] =
        sum[i][j];
    }
  }
}
