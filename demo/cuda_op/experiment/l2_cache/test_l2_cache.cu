...
auto main() noexcept(false) -> int {
  ...
  // 执行 FP16 矩阵乘法（内部自动调用 Tensor Core）
  // cublas_check(cublasGemmEx(
  //   handle,
  //   CUBLAS_OP_N,
  //   CUBLAS_OP_N,        // A 和 B 是否转置
  //   M,
  //   N,
  //   K,                  // 矩阵维度
  //   &alpha,             // alpha
  ...
  //   CUBLAS_GEMM_DEFAULT // 算法（自动选择 Tensor Core）
  //   ));
  ...
  print_device_buffer(b_ptr, K, N, "b_ptr");
  print_device_buffer(c_ptr, M, N, "c_ptr");
